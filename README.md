...
<h2>Description</h2>
In this lab, we’ll set up two virtual machines (VMs) in the same Virtual Network (VNet)—one as a Domain Controller (DC) with a static IP running Active Directory, and the other as a client machine. The client will join the domain, and we’ll configure its DNS to use the DC as the primary DNS server.

The goal of this project is to create a centralized system for managing user credentials and monitoring network activity. By routing all internet traffic through the Domain Controller, IT administrators can keep an eye on network behavior, track logs, and spot any suspicious activity. To make things even more efficient, we’ll use a PowerShell script to generate 1,000 user accounts, showcasing how easy it is to manage devices and users within the organization’s domain.

This setup not only simplifies user authentication but also strengthens security by giving admins better visibility into network traffic.
<br />
...
Start by creating a Resource Group in Azure. Next, set up two Virtual Machines (VMs)—one will act as the Domain Controller, and the other will be the Client. When creating the Domain Controller, assign it a name and make sure it belongs to the Resource Group you just created. 

<img src="https://i.imgur.com/uYfHMQG.png" height="70%" width="70%" alt="9"/><br />

For the operating system, select Windows Server 2022 as the image. It's recommended to choose a VM size with 2 vCPUs for optimal performance.
